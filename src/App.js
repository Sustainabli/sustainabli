...
function App(props) {
  ...
  const [availableSensors, setAvailableSensors] = useRecoilState(
    AVAILABLE_SENSORS_STATE
  ); // Available sensors to view metrics for the user
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      if (userInfo.role === USER_ROLE) {
        availableSensors = await fetchSensorInfoFromGroup(
          userInfo.organization_code,
          userInfo.group_name
        );
      } else if (userInfo.role === ORGANIZATION_ADMIN_ROLE) {
        availableSensors = await fetchSensorInfoFromOrganization(
          userInfo.organization_code
        );
      }
            ...
            <Col sm={12} lg={10} className="p-0">
              <Routes>
                    ...
                    <Route
                      ...
                      element={
                        <FumeHoodsPage
                          ...
                          updateSensors={setAvailableSensors}
                        />
                      }
                    />
                    <Route
                ...
                <Route
                  ...
                  element={<ProfilePage userInfo={userInfo} updateFumeHoods={setAvailableSensors}/>}
                />
