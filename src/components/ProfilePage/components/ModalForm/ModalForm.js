...
class ModalForm extends React.Component {
  ...
  componentDidMount = () => {
    const { selectedGroupInfo, selectedSensorInfo, selectedUserInfo } =
      this.props;
  ...
  onChangeSelectedGroup = (selected) =>
    this.setState({ groupQuery: selected.value });
  onChangeSelectedOrganizationCode = (selected) =>
    this.setState({ organizationCodeQuery: selected.value });
  onChangeSelectedSensors = (options) =>
    this.setState({
      selectedSensors: options.map((option) => ({
        id: option.value,
  ...
  onSubmitAddNewOrganization = async (event) => {
    ...
    const organizationCode =
      event.target.elements.organizationCodeFormGroup.value;
    const organizationName =
      event.target.elements.organizationNameFormGroup.value;

    const allOrganizations = await addOrganization(
      organizationCode,
      organizationName
    );
  ...
  onSubmitUpdateOrganizationInfo = async (event) => {
    ...
    const allOrganizations = await updateOrganizationInfo(
      newOrganizationCode,
      newOrganizationName,
  ...
  onSubmitCreateGroup = async (event) => {
    ...
    const allGroupsInOrganization = await addGroup(
      groupName,
  ...
  onSubmitUpdateGroupInfo = async (event) => {
    ...
    const allGroupsInOrganization = await updateGroupInfo(
      newGroupName,
  ...
  onSubmitAddOrganizationAdmin = async (event) => {
    ...
    const allOrganizationAdminUsers = await updateUserRole(
      email,
      ORGANIZATION_ADMIN_ROLE,
      organizationCodeQuery
    );
  ...
  onSubmitUpdateOrganizationAdminInfo = async (event) => {
    ...
    const allOrganizationAdminUsers = await updateOrganizationAdminInfo(
      newEmail,
  ...
  updateUserInfoAndUpdateUserList = async (
    newEmail,
    oldEmail,
    newGroupName,
    organizationCode,
    ...
    const allUsersInOrganization = await updateUserInfo(
      newEmail,
      oldEmail,
      newGroupName,
      organizationCode,
  ...
  onSubmitAddUserToOrganization = async (event) => {
    ...
    // TODO check for not found email
    this.updateUserInfoAndUpdateUserList(
      userEmail,
      userEmail,
      groupQuery,
      userInfo.organization_code,
  ...
  onSubmitUpdateUserInfo = async (event) => {
    ...
    this.updateUserInfoAndUpdateUserList(
      ...
      selectedUserInfo.email,
      groupQuery,
      selectedUserInfo.organization_code,
  ...
  onSubmitUpdateSensorInfo = async (event) => {
    ...
    const allSensors = await updateSensorInfo(
      newSensorId,
      selectedSensorInfo.id,
      organizationCodeQuery
    );
    this.updateSensorList(allSensors);
    this.onCloseModal();
  ...
  onSubmitUpdateFumeHoodInfo = async (event) => {
    ...
    const fumeHoodName = event.target.elements.fumeHoodNameFormGroup[0].value;
    const building = event.target.elements.fumeHoodNameFormGroup[1].value;
    const room = event.target.elements.fumeHoodNameFormGroup[2].value;
            ...
            <Modal.Body>
              ...
              <Form.Group>
                ...
                <Select
                  ...
                  isMulti={false}
                  onChange={(selected) =>
                    this.onChangeSelectedOrganizationCode(selected)
                  }
          ...
          <Form onSubmit={this.onSubmitAddUserToOrganization}>
            <Modal.Body>
              ...
              <Form.Group>
                ...
                <Select
                  options={allGroupsInOrganization.map((group) => ({
          ...
          <Form onSubmit={this.onSubmitCreateGroup}>
            <Modal.Body>
              <Form.Group controlId="groupNameFormGroup">
                ...
                <Form.Control type="input" placeholder="Group Name" required />
              </Form.Group>
              <Form.Group>
                <Form.Label>
                  <h5>Fume Hoods</h5>
                ...
                <Select
                  options={allFumeHoodsInOrganization.map((sensor) => ({
                    value: sensor.id,
          ...
          <Form onSubmit={this.onSubmitAddNewSensor}>
            <Modal.Body>
              <Form.Group controlId="addNewSensorIdFormGroup">
                ...
                <Form.Control type="input" placeholder="Sensor ID" required />
              </Form.Group>
              <Form.Group>
                <Form.Label>
                  <h5>Organization Code</h5>
            ...
            <Modal.Body>
              ...
              <Form.Group>
                ...
                <Select
                  ...
                  defaultValue={{
                    value: selectedUserInfo.organization_code,
              ...
              <Modal.Footer>
                <Button
                  variant="danger"
                  onClick={this.onRemoveOrganizationAdmin}
                >
                  Remove Organization Admin
                </Button>
                <Button variant="dark" type="submit">
                  Update Organization Admin Info
                </Button>
              </Modal.Footer>
            </Modal.Body>
          </Form>
            ...
            <Modal.Body>
              ...
              <Form.Group>
                ...
                <Select
                  ...
                  defaultValue={{
                    value: selectedSensorInfo.organization_code,
                    label: selectedSensorInfo.organization_code,
                  }}
                  closeMenuOnSelect={true}
                />
              </Form.Group>
        ...
        modalBody = (
          <Form onSubmit={this.onSubmitUpdateGroupInfo}>
                ...
                <Select
                  ...
                  defaultValue={selectedGroupInfo.sensor_infos.map(
                    (sensor_info) => ({
                      ...
                      label: sensor_info.fume_hood_name,
                    })
                  )}
                  isMulti={true}
                  onChange={(options) => this.onChangeSelectedSensors(options)}
            ...
            <Modal.Footer>
              ...
              <Button
                variant="danger"
                onClick={this.onRemoveGroupFromOrganization}
              >
        ...
        modalBody = (
          <Form onSubmit={this.onSubmitUpdateUserInfo}>
            <Modal.Body>
              ...
              <Form.Group>
                ...
                <Select
                  ...
                  defaultValue={{
                    value: selectedUserInfo.group_name,
                    label: selectedUserInfo.group_name,
                  }}
                  isMulti={false}
                  onChange={(selected) => this.onChangeSelectedGroup(selected)}
                  closeMenuOnSelect={true}
            ...
            <Modal.Footer>
              ...
              <Button
                variant="danger"
                onClick={this.onRemoveUserFromOrganization}
              >
