...
class SashChart extends React.Component {
  ...
  render() {
    ...
    const dataKeys = filteredData && filteredData.length ? Object.keys(filteredData[0]).filter(key => key !== 'time').filter(key => (key.includes("2368") || key.includes("2360") || key.includes("2364")) && (!key.includes("Total"))) : [];
